...
int AudioDevice_WaveOut::CanWrite(const AudioDevicePipe& pipe)
{
	...
	DWORD PlayCursor;
	Buffer->DSB->GetCurrentPosition(&PlayCursor, NULL);
	DWORD status;
	Buffer->DSB->GetStatus(&status);
...
bool AudioDevice_WaveOut::WaitEnd(const AudioDevicePipe& pipe)
{
	...
	if (PlayCursor>Buffer->WriteOffset)
	{
		...
		if (ret == WAIT_OBJECT_0)
		{
			...
			if (PlayCursor<Buffer->WriteOffset)
			{
				...
				DWORD dwBytes2;
				WaitForSingleObject(Buffer->Mutex, INFINITE);
				HRESULT res = Buffer->DSB->Lock(Buffer->WriteOffset, Buffer->BufferBytes - Buffer->WriteOffset, &lpvPtr1, &dwBytes1, &lpvPtr2, &dwBytes2, 0);
				if (DSERR_BUFFERLOST == res)
				{
					Buffer->DSB->Restore();
					res = Buffer->DSB->Lock(Buffer->WriteOffset, Buffer->BufferBytes - Buffer->WriteOffset, &lpvPtr1, &dwBytes1, &lpvPtr2, &dwBytes2, 0);
				}
				if (SUCCEEDED(res))
				{
					memset(lpvPtr1, 0, dwBytes1);
					if (NULL != lpvPtr2)memset(lpvPtr2, 0, dwBytes2);
					res = Buffer->DSB->Unlock(lpvPtr1, dwBytes1, lpvPtr2, dwBytes2);
				}
				Buffer->DSB->GetCurrentPosition(&PlayCursor, NULL);
				ReleaseMutex(Buffer->Mutex);
	...
	WaitForSingleObject(Buffer->Mutex, INFINITE);
	Buffer->DSB->Stop();
	Buffer->DSB->SetCurrentPosition(0);
	Buffer->WriteOffset = 0;
	Buffer->OutputWrittenPointer = 0;
	ReleaseMutex(Buffer->Mutex);
	return false;
...
bool AudioDevice_WaveOut::Stop(const AudioDevicePipe& pipe)
{
	...
	if (Buffer->DSB)
	{
		...
		Buffer->Mutex = NULL;
		SAFE_RELEASE(Buffer->DSB);
	}
	ZeroMemory(&Buffer->WFE, sizeof(Buffer->WFE));
	if (Buffer->DestroyDS)SAFE_RELEASE(m_DS);
	if (Buffer)
	{
		EnterCriticalSection(&cs);
