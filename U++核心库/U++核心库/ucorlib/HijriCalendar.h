...
namespace System
{
	...
	/// 回历日历类
	/// </summary>
	 class RUNTIME_API HijriCalendar : public Calendar
	{
	private:
		mutable int m_HijriAdvance;
	...
	protected:
		...
		Int64 DaysUpToHijriYear(int HijriYear) const;

		Int64 GetAbsoluteDateHijri(int y, int m, int d) const;
	...
	public:
		...
		/// </summary>
		/// <returns></returns>
		int GetHijriAdjustment() const;

		/// <summary>
		/// 设置要在日历中添加或减去的天数
		/// </summary>
		/// <param name="value">增益天数 (-2 ~ 2)</param>
		/// <returns></returns>
		void SetHijriAdjustment(int value);
		...
		/// <returns></returns>
		void SetTwoDigitYearMax(int value);

		/// <summary>
		/// 创建<see cref="HijriCalendar"/>实例.
		/// </summary>
		HijriCalendar();
