...
namespace System
{
	...
	/// 日本阳历类
	/// </summary>
	 class  RUNTIME_API JapaneseCalendar : public Calendar
	{
	protected:
		friend class JapaneseLunisolarCalendar;
		...
		static wstringvector AbbrevEraNames();

		static wstringvector EnglishEraNames();

		static wstringvector EraNames();

		static std::vector<EraInfo> GetEraInfo();

		bool IsValidYear(int year, int era)  const override;

		int GetWeekOfYear(const DateTime& time, CalendarWeekRule rule, DayOfWeek firstDayOfWeek) const override;
	...
	public:
		...
		/// <summary>
		/// 创建<see cref="JapaneseCalendar"/>实例.
		/// </summary>
		JapaneseCalendar();
		...
		REFLECT_CLASS(JapaneseCalendar)
	};
}